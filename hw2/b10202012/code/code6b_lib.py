...
class Packet:
    ...
    def create(message, send_to: int, pk):
        ...
        message = message.ljust(400-32, b'\x00')
        one_time_key = int(randbytes(8).hex(), 16)
        encrypted_key = PublicKeyCipher.encrypt(pk, one_time_key)
        cipher = StreamCipher.encrypt(one_time_key, message)
        data = encrypted_key + cipher
        return Packet(data)

    def add_next_hop(self, target, pk):
        # TODO
        self.data = self.data[:-32-20]
        send_to = target.ljust(20, b' ')
        one_time_key = int(randbytes(8).hex(), 16)
        encrypted_key = PublicKeyCipher.encrypt(pk, one_time_key)
        cipher = StreamCipher.encrypt(one_time_key, send_to + self.data)
        self.data = encrypted_key + cipher
